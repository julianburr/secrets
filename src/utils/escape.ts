...
export const joinCommand = (filePath: any, rawArguments: any) => {
...
export const escapeLines = (lines: any) =>
  stripVTControlCharacters(lines)
    .split('\n')
    .map((line) => escapeControlCharacters(line))
    .join('\n');

const escapeControlCharacters = (line: any) =>
  line.replaceAll(SPECIAL_CHAR_REGEXP, (character: any) => escapeControlCharacter(character));

const escapeControlCharacter = (character: any) => {
  const commonEscape = COMMON_ESCAPES[character as keyof typeof COMMON_ESCAPES];
...
const quoteString = (escapedArgument: any) => {
